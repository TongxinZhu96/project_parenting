...
```
The statistical procedure adopted for this study is cluster analysis, which tries to classify observations into mutually exclusive groups so that the differences within the same group are as small as possible while the differences between groups are as large as possible .  Several algorithms are available for clustering, and the method used for this paper is K-mean clustering, in which k is a pre-specified number, and means referring to the mean of the observations assigned to the same cluster corresponding to the center of the cluster. K-means algorithm was chosen for this study because it can be applied to both continuous and categorical variables . Before cluster analysis, I standardized the 24 input variables but Child ID for comparability. 
